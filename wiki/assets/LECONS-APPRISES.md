...
## 1. Architecture et philo Code (PHP / OOP)
...
### Sécurité et standards WP (WP.org)
...
## 2. Intégration avancée avec Woo

### Manipulation de l'interface d'admin Woo
...
## 3. Transition vers le Site Editor (FSE) & Gutenberg
...
- **L'anomalie** : Le plugin `QueryMonitor` a remonté une alerte fatale depuis WP 6.7.0 : `Function _load_textdomain_just_in_time was called incorrectly`.
...
### Phase 1 : Validation manuelle (WordPress.org) et release manuelle pour Github
- Soumettre initialement à WordPress.org requiert une archive `.zip` propre.
- **Solution locale** : Un script `build-zip.sh` purge tous les éléments de développement (`.git`, `tests`, `phpcs.xml`, `wiki/`, `.vscode`, etc.).
- **Versioning "Rang 3"** : Le script interactif `release.sh` automatise le calcul de la version avec une limite mathématique (les mineures/patchs reviennent à zéro après `3`, ex: `1.3.3` -> `2.0.0`).
- **Push interactif** : En fin de process, le script propose un push asynchrone sur les remotes configurés (`origin` pour GitLab et `github` pour le miroir GitHub).
- **Rollback de secours** : En cas d'erreur de versioning, la manœuvre de secours consiste à détruire les tags Git (`git tag -d` et `git push --delete`) et annuler le commit (`git reset --hard HEAD~1`) pour libérer la version.

### Phase 2 : Déploiement Continu SVN (GitHub Actions)
- Le dépôt local est connecté à un miroir GitHub destiné à piloter le déploiement continu **après l'approbation** par WordPress.
- **Leçon** : L'utilisation de l'action `10up/action-wordpress-plugin-deploy` permettra de lier directement les tags Git au répertoire SVN officiel de WordPress.org, compilant les assets Gutenberg à la volée et automatisant la mise à jour des pages de présentation, sans plus avoir besoin de soumettre de `.zip` manuel.
